...
ASSETS = {
    "BTCUSDT": ["5m", "15m", "1h"],
    "PAXGUSDT": ["5m", "15m", "1h"],  # Binance Gold-backed token
}
...
def get_volatility_level(series):
    atr_val = ATR(series)
    std_val = series.std()
    if atr_val < std_val * 0.5:
        return "Low", 0.2, "⬇️"
    elif atr_val < std_val * 1.2:
        return "Medium", 0.25, "⚖️"
    else:
        return "High", 0.35, "⬆️"
...
def get_signal(series):
    ...
    return {
        ...
        "vel_icon": vel_icon,
        "pred_buy_low": pred_buy_low,
        "pred_buy_high": pred_buy_high,
        "pred_sell_low": pred_sell_low,
        "pred_sell_high": pred_sell_high
    }
...
# ---------------------------
def main_loop():
    send_telegram("🚀 DicksonBTC Trend Bot Started! Watching BTCUSDT + Gold (PAXGUSDT)...")
        ...
        for symbol in ASSETS:
            for interval in ASSETS[symbol]:
                if interval=="5m" and minutes % 5 != 0:
                    continue
                if interval=="15m" and minutes % 15 != 0:
                    continue
                if interval=="1h" and now.minute != 0:
                    continue
                
                current_key = f"{now.hour}-{minutes//int(interval[:-1])}"
                if last_sent[symbol][interval] == current_key:
                    continue
